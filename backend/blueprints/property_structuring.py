...
# app = Flask(__name__)
# CORS(app, resources={r"/send_manuscripts": {"origins": "http://localhost:3001"}})
...
@structure_bp.route('/api/send_manuscripts', methods=['POST'])
def send_manuscripts():
    """
    ...
    data = request.get_json(force=True)                 # <- robust parsing
    current_app.logger.info(                            # <- pretty log entry
        "⇢ Received from Vue:\n%s",
        json.dumps(data, indent=2, ensure_ascii=False)  # keep accents intact
    )
    ...
    return jsonify({"structured_results": results}), 200
